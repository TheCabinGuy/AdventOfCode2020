...
let dataInput = [...originalDataInput];

let seatsChanged = false;

function getAdjacentSeats(currentRow, aboveRow, belowRow, index){
  return [
    currentRow[index - 1],
    currentRow[index + 1],
    aboveRow[index],
    ...
    belowRow[index + 1],
    belowRow[index - 1]
  ].reduce((total, seat) => seat === "#" ? total + 1 : total, 0)
}

do{
  seatsChanged = false;
  const tempSeating = [...dataInput];
  for(const [rowIndex, row] of dataInput.entries()) {
    for(const [index, column] of row.split("").entries()) {
      const noOfOccupiedSeatsAdjacent = getAdjacentSeats(row, dataInput[rowIndex - 1] || "", dataInput[rowIndex + 1] || "", index);
      if(noOfOccupiedSeatsAdjacent >= 4 && column === "#"){
        let tempRow = tempSeating[rowIndex].split("");
        tempRow[index] = "L";
        tempSeating[rowIndex] = tempRow.join("");
        seatsChanged = true;
      }
      if(noOfOccupiedSeatsAdjacent === 0 && column === "L"){
        let tempRow = tempSeating[rowIndex].split("");
        tempRow[index] = "#";
        tempSeating[rowIndex] = tempRow.join("");
        seatsChanged = true;
  ...
  dataInput = [...tempSeating];
}while(seatsChanged)

const answer = dataInput.reduce((total, row) => {
  return total + row.split("").reduce((columnTotal, column) => column === "#" ? columnTotal + 1 : columnTotal, 0)
}, 0)

console.log(answer);

dataInput = [ ...originalDataInput ];

function checkDirection(rowIndex, columnIndex, data, rowChange, columnChange){
  let currentSeat = data[rowIndex][columnIndex];
  let row = rowIndex;
  let column = columnIndex;
  do{
    row+=rowChange;
    column+=columnChange;
    currentSeat = dataInput[row]?.[column]
  }while(currentSeat && currentSeat !== "#" && currentSeat !== "L")
  return currentSeat;
}

function getNoOfSeenSeating(rowIndex, columnIndex, data){
  return [
    checkDirection(rowIndex, columnIndex, data, 0, -1),
    checkDirection(rowIndex, columnIndex, data, 0, 1),
    checkDirection(rowIndex, columnIndex, data, 1, 0),
    checkDirection(rowIndex, columnIndex, data, 1, 1),
    checkDirection(rowIndex, columnIndex, data, 1, -1),
    checkDirection(rowIndex, columnIndex, data, -1, 0),
    checkDirection(rowIndex, columnIndex, data, -1, 1),
    checkDirection(rowIndex, columnIndex, data, -1, -1)
...
do{
  ...
  for(const [rowIndex, row] of dataInput.entries()) {
    for(const [index, column] of row.split("").entries()) {
      const noOfOccupiedSeatsAdjacent = getNoOfSeenSeating(rowIndex, index, dataInput);
      if(noOfOccupiedSeatsAdjacent >= 5 && column === "#"){
        let tempRow = tempSeating[rowIndex].split("");
        tempRow[index] = "L";
        tempSeating[rowIndex] = tempRow.join("");
        seatsChanged = true;
...
const answer2 = dataInput.reduce((total, row) => {
  return total + row.split("").reduce((columnTotal, column) => column === "#" ? columnTotal + 1 : columnTotal, 0)
}, 0)
