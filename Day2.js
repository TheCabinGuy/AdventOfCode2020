...
const answer = dataInput.filter(val => {
  const [policy, password] = val.split(":");
  const [minMax, policyLetter] = policy.split(" ");
  const [min, max] = minMax.split("-").map(Number);
  const numberOfValidLetters = password.split("").filter(letter => letter === policyLetter).length;
  return numberOfValidLetters >= min && max >= numberOfValidLetters;
}).length
console.log(answer);

const answer2 = dataInput.filter(val => {
  const [policy, password] = val.split(":").map(a => a.trim());
  const [positions, policyLetter] = policy.split(" ");
  const [position1, position2] = positions.split("-").map(Number);
  return (password[position1 - 1] === policyLetter) !== (password[position2 - 1] === policyLetter);
}).length;
console.log(answer2);