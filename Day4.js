...
for(const value of dataInput){
  if(value === ""){
    ...
    const pairs = value.split(" ").map(a => a.trim());
...
const answer = formattedData.reduce((total, passport) => {
  const isValidPassport = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"].every(key => Object.keys(passport).includes(key));
  return isValidPassport ? total + 1 : total;
}, 0);

console.log(answer);

const answer2 = formattedData.reduce((total, passport) => {
  const isValidPassport = [
    ...
    ["hcl", /^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$/],
    ["ecl", /^amb|blu|brn|gry|grn|hzl|oth$/],
    ["pid", { test: value => value.length === 9}]
  ].every(([key, pattern]) => Object.keys(passport).includes(key) && pattern.test(passport[key]));
  return isValidPassport ? total + 1 : total;
}, 0);
