...
let formattedData = Array.from({length: dataInput.length}, () => [])

for(const value of dataInput){
  if(value === ""){
    index+=1;
  } else {
    formattedData[index].push(value)
  }
}

formattedData = formattedData.filter(arr => arr.length !== 0);

const answer = formattedData.map(group => (
  group
    .reduce((total, current) => [...total, ...current.split("").filter(a => !total.includes(a))], [])
    .length
)).reduce((total, curr) => total + curr, 0)

console.log(answer);

const answer2 = formattedData.map(group => {
  const formattedGroup = group.reduce((total, current) => [...total, ...current.split("")], []);
  if(group.length === 1){
    return formattedGroup.length;
  } else {
    return formattedGroup
      .filter((value, index,self) => self.filter(a => a === value).length === group.length)
