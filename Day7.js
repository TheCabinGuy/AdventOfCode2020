...
const rules = dataInput.reduce((total, value) => {
  const [bag, rules] = value.split("contain");
  const splitRules = rules.split(",").map(rule => (
    rule
      .replace(".", "")
      .replace("bags", "bag")
      .slice(2)
  ...
  return {...total, [bag.replace("bags", "bag").trim()]: splitRules}
}, {})

const canContainShinyGoldBag = (bag) => {
  const childrenBags = rules[bag];
  if(typeof childrenBags === "undefined"){
    return false;
  }
  if(childrenBags.some(childBags => childBags.includes("shiny gold")) || bag === "shiny gold"){
    return true;
  } else {
    return childrenBags.some(canContainShinyGoldBag);
  }
}

const answer = Object.keys(rules).filter(canContainShinyGoldBag).length
...
const rulesWithAmounts = dataInput.reduce((total, value) => {
  const [bag, rules] = value.split("contain");
  const splitRules = rules.split(",").map(rule => {
    const bagNameWithNumber = rule
      .replace(".", "")
      .replace("bags", "bag")
      .trim()

    return {number: Number(bagNameWithNumber.split(" ", 1)) || 0, bagName: bagNameWithNumber.slice(2)}
  });
  return {...total, [bag.replace("bags", "bag").trim()]: splitRules}
}, {})

const howManyChildBagsDeep = (total, bag) => {
  const childrenBags = rulesWithAmounts[bag];
  if(typeof childrenBags === "undefined"){
    return total;
  }
  const { numberOfChildrenBags, childrenBagNames = [] } = childrenBags.reduce(
    (subTotal, {number, bagName}) => (
      {
        numberOfChildrenBags: subTotal.numberOfChildrenBags + number,
        childrenBagNames: [...subTotal.childrenBagNames, ...Array.from({length: number}, () => bagName)]
      }
    ), { numberOfChildrenBags: 0, childrenBagNames: []}
  );
  return numberOfChildrenBags + childrenBagNames.reduce(howManyChildBagsDeep, total);
}

const answer2 = howManyChildBagsDeep(0, "shiny gold bag");
